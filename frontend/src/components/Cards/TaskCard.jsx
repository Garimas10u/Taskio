...
const TaskCard = ({
     title, description, priority, status, progress, createdAt, dueDate, assignedTo, attachmentCount, completedTodoCount,
    ...
    const getStatusTagColor = () => {
        switch(status) {
            ...
            case "In Progress":
                return "text-blue-500 bg-blue-50 border border-blue-500/20";
  ...
  return (
    <div className='bg-white rounded-xl py-4 shadow-md shadow-gray-100 border border-gray-200/50 cursor-pointer ' onClick={onClick}>
        <div className='flex item-end gap-3 px-4'>
        ...
        <div className={`px-4 border-l-[3px] ${status ==="In Progress"? "border-cyan-500" : status ==="Completed" ? "border-indigo-500" : "border-violet-500"}`}>
            ...
            <p className='text-[13px] text-gray-700/80 font-medium mb-2 mt-2 leading-[18px]' >{todoChecklist?.length > 0 ? (
                        <>
                        Task Done:{" "}
                        ...
                        <span className='italic text-gray-500'>No checklist</span>
                    )}
            </p>
